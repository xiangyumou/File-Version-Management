...
public:
  MOCK_METHOD(FileSystem &, get_file_system, (), (override));
  MOCK_METHOD(std::vector<std::string>, get_current_path, (),
              (const, override));
  MOCK_METHOD(std::string, get_current_path_string, (), (const, override));
  MOCK_METHOD(bool, set_current_path, (const std::vector<std::string> &path),
              (override));
  MOCK_METHOD(std::vector<std::string>, get_previous_path, (),
              (const, override));
...
TEST_F(CdCommandTest, InvalidParams) {
  ...
  std::vector<std::string> params = {};
  auto result = cmd.execute(session, params);
  EXPECT_FALSE(result.success); // Validation should fail
