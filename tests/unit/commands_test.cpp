...
protected:
  ...
  void SetUp() override {
    // Setup initial storage state: empty/fail load to trigger fresh
    // initialization
    EXPECT_CALL(mock_storage, load(_, _, _)).WillRepeatedly(Return(false));

    // Initialization triggers creation of "root"
    EXPECT_CALL(mock_node_manager, get_new_node("root"))
        .WillRepeatedly(Return(1));
    EXPECT_CALL(mock_node_manager, get_name(_))
        .WillRepeatedly(Return("root")); // Catch-all for logging
    EXPECT_CALL(mock_node_manager, increase_counter(_))
        .Times(testing::AnyNumber());

    fs = std::make_unique<FileSystem>(&mock_logger, &mock_node_manager,
                                      &mock_storage);
    session = std::make_unique<Session>(*fs);
  }
};
...
TEST_F(CommandsTest, TouchCommandCreatesFile) {
  ...
  EXPECT_CALL(mock_node_manager, get_new_node("new_file.txt"))
      .WillOnce(Return(2));
  EXPECT_CALL(mock_node_manager, get_name(2))
      .WillRepeatedly(Return("new_file.txt")); // For verification if needed

  CommandResult result = cmd.execute(*session, params);
...
TEST_F(CommandsTest, CdCommandChangesDirectory) {
  ...
  mkdir_cmd.execute(*session, {"dir1"});

  // 2. Cd
  CdCommand cd_cmd;
  CommandResult result = cd_cmd.execute(*session, {"dir1"});
